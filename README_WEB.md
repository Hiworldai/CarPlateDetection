...
如果对方需要测试视频和图片，可以保留 `TestFiles/`。如果想让压缩包更小，可以不带 `datasets/` 和 `runs/`，它们主要用于训练和历史输出，不是网页识别必需项。
...
这个项目适合在简历里定位为一个“AI 识别能力 Web 化”的工程项目。你不需要把它说成自己从零训练了所有模型，更稳的说法是：基于已有 YOLO 车牌检测模型和 PaddleOCR 文字识别能力，完成前后端分离改造、识别流程封装、游客/登录双模式、数据导出和部署方案。
...
> 这是一个车牌识别 Web 系统。我把原来偏脚本式的 Python 识别流程封装成 FastAPI 服务，前端用 Vue 做上传、摄像头采集、任务进度、结果列表和导出。识别流程上使用 YOLO 定位车牌区域，再用 PaddleOCR 识别车牌字符，并针对蓝牌、绿牌、黄牌等场景做了颜色候选、OCR 兜底和车牌格式纠错。数据层支持游客本地存储和登录后服务器存储两种模式，方便展示和保护用户隐私。
...
答：浏览器摄像头 API 通常要求安全上下文，也就是 localhost 或 HTTPS。公网 HTTP 页面可能无法调用摄像头，所以正式上线要配置域名和 HTTPS。
...
```text
车牌识别 Web 系统：使用 Vue 3 + FastAPI 将 YOLO + PaddleOCR 车牌识别流程 Web 化，支持图片、视频、摄像头识别；实现游客 localStorage 存储与登录 MySQL 持久化两种模式，支持筛选列表、CSV/Excel 导出和 Docker 部署；针对蓝牌识别失败问题加入颜色候选、OCR 兜底和车牌格式纠错，提高复杂场景下的识别稳定性。
```
